...
{
    public class WhenCallingAcos : IClassFixture<ProcessedTableRepositoryFixture>
    {
        [Theory]
        ...
        public void ShouldConvertToAbsoluteValue(double value, double expected)
        {
            ...
            {
                FixedPoint.ConvertToDouble(MathFixedPoint.Acos(FixedPoint.From(value))).Should().Be(double.NaN);
            }
            else
            {
                FixedPoint.ConvertToDouble(MathFixedPoint.Acos(FixedPoint.From(value))).Should().BeApproximately(Math.Acos(expected), 0.05f);
            }
        }
    }
}