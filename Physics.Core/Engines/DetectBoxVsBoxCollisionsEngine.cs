...
namespace Physics.Core.Engines
{
    ...
    public class DetectBoxVsBoxCollisionsEngine : IQueryingEntitiesEngine, IScheduledPhysicsEngine
    {
        ...
        public DetectBoxVsBoxCollisionsEngine(IEngineScheduler engineScheduler)
        {
            ...
            foreach (var ((transforms, colliders, manifolds, rigidbodies, count), _) in entitiesDB.QueryEntities<TransformEntityComponent, BoxColliderEntityComponent, CollisionManifoldEntityComponent, RigidbodyEntityComponent>(GameGroups.RigidBodyWithBoxColliderGroups))
            {
                for (var a = 0; a < count; a++)
                {
                    ...
                    for (var b = a + 1; b < count; b++)
                    {
                        ...
                        Console.WriteLine($"box collision {a} vs {b}");
        ...
        private static CollisionManifold? CalculateManifold(int indexA, AABB a, int indexB, AABB b)  {
            ...
            {
                return CollisionManifold.From(min, penetration.Value.Normalize(), CollisionType.AABBToAABB, indexA, indexB);
            }
