...
namespace FixedMaths.Core
{
    ...
    public readonly struct FixedPoint
    {
        ...
        private const int K = 1 << (Q - 1);
        private static readonly int IntegerMaxValue = (int)Math.Pow(2, sizeof(int) * 8 - 1 - Q) - 1; // -1 is because int is singed
        private static readonly int IntegerMinValue = -(int) Math.Pow(2, sizeof(int) * 8 - 1 - Q);   // -1 is because int is singed
        private static readonly int FractionalMaxValue = (int)Math.Pow(2, Q) - 1;
        ...
        public static FixedPoint Kludge  = new FixedPoint(1);// Used to avoid division by zero errors, by adding a tiny tiny bit of a number
        
        public static int ConvertToInteger(FixedPoint fp)
        {
            return fp.Value >> Q;
        }
        ...
        public static FixedPoint FromExplicit(int value)
        {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private FixedPoint(int value, bool isNaN = false)
        {
            Value = value;
            _isNaN = isNaN;
        }
        ...
        {
            return obj is FixedPoint other && Equals(other);
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public override int GetHashCode()
        {
            return HashCode.Combine(Value, _isNaN);
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public override string ToString()
        {
            return _isNaN ? "NaN" : $"{ConvertToFloat(this):0.0000}";
        }
    }
}
